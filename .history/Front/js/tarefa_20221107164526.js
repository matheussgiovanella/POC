...
const taskDelete = async (id) => {
    ...
    await axios.delete(`${ENDPOINT}tarefas/` + id)
        .then((response) => {
            Swal.fire(`Task '${data.descricao}' deleted`)
            loadTable();
        }, (error) => {
...
const SendEmail = async () => {
    const pdf = await axios.get(`${ENDPOINT}pdf`)
    Swal.fire(
        {
            title: `Send Pdf`,
            html:
            ...
            preConfirm: async () =>
            {
                try
                {
                    const email_user = document.getElementById("emai")
                    const email_to = email_user;
                    var mailOptions =
                    {
                        from: email_user,
                        to: email_to,
