...
export class tarefasController
{
    ...
    _validateData = async (data : any, id? : string) =>
    {
        ...
        for (const attribute of requiredAttributes)
        {
            ...
            if (await this._checkIfSituacaoExists(data.situacao, id))
            {
                throw new Error(`The tarefa with situacao ${data.situacao} already exists.`)
            }
            newTarefa[attribute] = data[attribute]
        }
        
        return newTarefa
    }
	_checkIfSituacaoExists = async (situacao : any, id? : string) =>
    {
        let where = {}
        ...
        const count = await Tarefa.count({
            where: where
        })
