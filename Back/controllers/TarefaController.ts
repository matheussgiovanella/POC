...
export class tarefasController
{
    ...
    generatePDF = async (req : Request, res : Response, next : NextFunction) =>
    {
        ...
        html += `<h1 style="text-align: center;">Relatório de Tarefas</h1>`
        ...
        for (const tarefa of tarefas)
        {
            ...
            html += `<td>${values.data_criacao}</td>`
            html += `<td>${values.data_vencimento}</td>`
            html += `<td>${values.descricao}</td>`
            html += `<td>${values.situacao}</td>`
            html += `<td>${values.prioridade}</td>`
